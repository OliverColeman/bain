...
public class STDPTestGUI extends JFrame {
	...
	public STDPTestGUI() {
		...
		goButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Synapse synapse = synapseSettings.getSynapse();
	...
	private class SpikeProtocolSettingsPanel extends JPanel {
		...
		public SpikeProtocolSettingsPanel(final STDPTestGUI gui) {
			...
			final SpikeTimingSetter[][] spikeTimingSetters = new SpikeTimingSetter[maxSpikePatternVariationDimensions+1][2]; //[dimension][pre, post]
	        ...
	        GridBagConstraints gbc = new GridBagConstraints();
	        gbc.weightx = 1; gbc.weighty = 1; gbc.gridx = 0; gbc.gridy = 0; gbc.anchor = GridBagConstraints.NORTH; gbc.fill = GridBagConstraints.HORIZONTAL;
		...
		public SpikeProtocolSettings getSpikeProtocolSettings() {
			...
			int[][] variationDimForSpike = new int[2][Math.max(settings.spikeCounts[0], settings.spikeCounts[1])]; //[pre, post][spike index]
						...
						for (int si = 0; si < settings.spikeCounts[p]; si++) {
							...
							if (settings.patterns[0][p][si] != settings.patterns[d][p][si]) {
								...
								if (variationDimForSpike[p][si] != 0) {
									JOptionPane.showMessageDialog(null, "A spikes timing may vary at most over one variation dimension. " + (p == 0 ? "Pre" : "Post") + "-synaptic spike " + (si + 1) + " varies over two.", "Error", JOptionPane.ERROR_MESSAGE);
								}
	...
	private class SynapseSettingsPanel extends JPanel {
		...
		public SynapseSettingsPanel(final STDPTestGUI gui){
			...
			synapseSelector.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					try {
						...
						JOptionPane.showMessageDialog(null, "Unable to load selected synapse type.", "Error", JOptionPane.ERROR_MESSAGE);
						e1.printStackTrace();
						return;
					}
				}
			...
			GridBagConstraints gbc = new GridBagConstraints();
			gbc.weightx = 1; gbc.weighty = 0; gbc.gridx = 0; gbc.gridy = 0; gbc.anchor = GridBagConstraints.NORTH; gbc.fill = GridBagConstraints.HORIZONTAL;
		...
		private void setupSynapseConfig(JPanel synapseParamsPanel, Synapse synapse) {
			...
			if (config != null) {
				...
				if (paramNames != null) {
					GridBagConstraints gbc = new GridBagConstraints();
			        gbc.weightx = 1; gbc.weighty = 1; gbc.gridx = 0; gbc.gridy = 0; gbc.gridwidth = 2; gbc.anchor = GridBagConstraints.NORTH; gbc.fill = GridBagConstraints.HORIZONTAL;
								...
								if (index < presetNames.length) {
									try {
										...
										for (int pi = 0; pi < params.length; pi++) {
											...
											// This prevents an infinite loop when the text field event listener 
											// recognises that the text fields contain the same values a preset
					...
					for (String param: paramNames) {
						...
						fields[pi].getDocument().addDocumentListener(new DocumentListener() {
							@Override
							public void changedUpdate(DocumentEvent arg0) { changeOccurred(); }
							@Override
							public void insertUpdate(DocumentEvent arg0) { changeOccurred(); }
							@Override
							public void removeUpdate(DocumentEvent arg0) { changeOccurred(); }
	...
	private static JFreeChart createChart(TestResults results, int timeResolution) {
		...
		if (results.getType() == STDPTest.RESULT_TYPE_STDP) {
			...
			for (String label: results.getResultLabels()) {
				if (!label.startsWith("Time")) {
					plotData.addSeries(label, results.getResult("Time", label));
				}
			}
		...
		else if (results.getType() == STDPTest.RESULT_TYPE_STDP_2D) {
			double[][] data = results.getResult("Time delta 1", "Time delta 2", "Strength");
	...
	private static class SpikeTimingSetter extends Box implements ChangeListener, ThumbListener {
		...
		// Make sure spike timings are in same order temporally as their indexes.
		...
		private static class SliderTrackRenderer extends JComponent implements TrackRenderer {
		    ...
		    protected void paintComponent(Graphics g) {
		        g.setColor(Color.BLACK); g.fillRect(0, 0, slider.getWidth(), slider.getHeight()-3);
		        //int zeroPoint = (int) Math.round(((double) -spikeTimingSetter.minValue/spikeTimingSetter.period) * slider.getWidth());
		        //g.setColor(Color.lightGray); g.drawLine(zeroPoint, 0, zeroPoint, slider.getHeight()-3);
 		    }
		    @Override
		    public JComponent getRendererComponent(JXMultiThumbSlider slider) {
		        this.slider = slider; return this;
		    }
		}
		private static class SliderThumbRenderer extends JComponent implements ThumbRenderer {
		    ...
		    protected void paintComponent(Graphics g) {
		    	...
		    	// thumb size is currently forced to 10x10, work around to use thumb width of 4.
